...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--bfs", action="store_true", help="Run breadth-first search"
    )
    parser.add_argument(
        "--dfs", action="store_true", help="Run depth-first search"
    )
    ...
    parser.add_argument(
        "--seed", type=int, required=True, help="Seed the PRNG"
    )
    ...
    # You may wish to add your own tests and etc.
        ...
        print("Goal: ", str(goal))
        print("Path (or costs for Dijkstra): ")
        for i in range(path.get_size()):
            print(path[i].get_key(), path[i].get_value(), end=", ")
